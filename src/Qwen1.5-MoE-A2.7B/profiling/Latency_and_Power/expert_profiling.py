"""
Expert Selection Profiling for Qwen1.5-MoE-A2.7B

This script profiles the latency and power consumption of each layer and selected experts in the Qwen1.5-MoE-A2.7B model.
It uses Hugging Face Transformers and Accelerate for efficient model loading with disk offloading, and measures both
CPU/GPU timings and power usage for each layer. The script also filters results to report only on the experts that were
actually selected by the model's gating mechanism during inference.

* This script needs to be run with sudo privileges to access the power monitoring resources.
* run_llm_python.sh can be used to achieve this.
...
- Filters results to keep only the selected experts per MoE block.
- Writes a detailed profiling report to a text file.
"""
...
# Remove hooks after profiling run
for hook in hooks:
    hook.remove()

# Power consumption profiling: repeat each layer's forward pass and measure power
NUM_REPEATS = 50  # Number of times to repeat each layer
...
# Filter results to keep only the experts that were selected by the gating mechanism
print("Filtering results for selected experts...")
...
for layer_name in activations.keys():
    ...
    expert_match = expert_pattern.fullmatch(layer_name)
    ...
    if gate_match:
        ...
        topk_by_layer[layer_idx] = topk.indices  # Save for corresponding expert layer

    elif expert_match:
        # Only keep expert results if this expert was selected by the gate
        layer_idx = expert_match.group(1)
        expert_idx = int(expert_match.group(2))
        if layer_idx in topk_by_layer:
            topk_experts = topk_by_layer[layer_idx]  # tensor [batch_size, 4]

            # Check if expert_idx is selected for any sample in batch
            selected_mask = (topk_experts == expert_idx).any(dim=1)  # shape: [batch_size]

            # Only keep samples where this expert was selected
            if selected_mask.any():
                expert_results[layer_name] = results[layer_name]

    else:
        # For non-expert layers, keep all results
        expert_results[layer_name] = results[layer_name]


# Write profiling results to a file
print("Writing results to file...")
output_file = "Measurements/expert_selection_profiling.txt"
with open(output_file, "w") as f:
    f.write(
        f"{'Layer':<45} {'Output Shape':<30} {'GPU Latency (ms)':<20} {'CPU Latency (ms)':<20} {'GPU Power (mW)':<20} {'CPU Power (mW)':<20}\n"
    )
    f.write("=" * 155 + "\n")
    for layer_name in expert_results.keys():
        f.write(
            f"{layer_name:<45} {str(activations[layer_name].shape):<30} {expert_results[layer_name]['gpu_time']:<20f} {expert_results[layer_name]['cpu_time']:<20f} {expert_results[layer_name]['avg_gpu_power']:<20f} {expert_results[layer_name]['avg_cpu_power']:<20f}\n"
        )
print(f"Results written to {output_file}")

# Sum up and print the execution times of all layers
total_gpu_time = sum(expert_results[layer_name]["gpu_time"] for layer_name in expert_results.keys())
total_cpu_time = sum(expert_results[layer_name]["cpu_time"] for layer_name in expert_results.keys())
